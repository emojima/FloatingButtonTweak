    ...
    [self.urlReplacementRules addObject:@{
        ...
        @"rules": @[
            ...
            @{
                ...
                @"replacement": @";tweak_trace(this,arguments,'enemyRoot');},$1.$2=function(){this.sortEnemyRootList=[]",
                @"useRegex": @YES
            },
            @{
                @"urlPattern": @"bdpfile://bd\\.timor\\.wk/.*/main/game\\.js",
                ...
                @"replacement": @",tweak_trace(this,arguments,'game_init');},$1.onUpdateSQEnemy=function(",
                @"useRegex": @YES
            }
        ]
    }];
    ...
    [self.urlReplacementRules addObject:@{
        ...
        @"rules": @[
            @{
                ...
                @"urlIsRegex": @YES,
                @"contentPattern": @"__tt_define__(",
                @"replacement": @"(function(){const REPORT_URL='bdpfile://bd.timor.wk/helloworld';const methods=['log','info','warn','error'];methods.forEach(method=>{const originalMethod=console[method];console[method]=function(...args){if(originalMethod){originalMethod.apply(console,args)}try{const content=args.map(arg=>{if(typeof arg==='object'){return JSON.stringify(arg,(key,val)=>{if(val!==null&&typeof val==='object'){if(seen.has(val))return'[Circular]';seen.add(val)}if(typeof val==='bigint')return val.toString()+'n';if(typeof val==='function')return'[Function]';if(typeof val==='undefined')return'[Undefined]';if(typeof val==='symbol')return val.toString();return val})}return String(arg)}).join(' ');const query=`type=${method}&msg=${encodeURIComponent(content)}`;const finalUrl=`${REPORT_URL}?${query}`;const img=new Image();img.src=finalUrl}catch(e){if(originalMethod){originalMethod.call(console,'Log report error:',e)}}}})})();__tt_define__(",
                @"useRegex": @NO
            }
        ]
    }];
