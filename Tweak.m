    ...
    [self.urlReplacementRules addObject:@{
        ...
        @"rules": @[
            @{
                ...
                @"urlIsRegex": @YES,
                @"contentPattern": @"__tt_define__(",
                @"replacement": @"(function(){'use strict';const ENDPOINT='bdpfile://bd.timor.wk/helloworld';const MAX_MSG=2000;const MAX_URL=18000;const FLUSH_MS=200;const MAX_QUEUE=50;const queue=[];let timer=null;let unloading=false;const stringify=(v)=>{try{return JSON.stringify(v,(key,val)=>{if(val!==null&&typeof val==='object'){if(seen.has(val))return'[Circular]';seen.add(val)}if(typeof val==='bigint')return val.toString()+'n';if(typeof val==='function')return'[Function]';if(typeof val==='undefined')return'[Undefined]';if(typeof val==='symbol')return val.toString();return val})}catch{return String(v)}};const toText=(args)=>args.map(a=>{if(a===null)return'null';if(a===undefined)return'undefined';if(typeof a==='object')return stringify(a);return String(a)}).join(' ');const cut=(s,n)=>s.length>n?s.slice(0,n)+'…':s;const send=(msgs)=>{if(!msgs.length)return;try{let text=msgs.join('\n');const safe=Math.floor((MAX_URL-ENDPOINT.length-50)/3);if(text.length>safe)text=cut(text,safe);new Image().src=`${ENDPOINT}?msg=${encodeURIComponent(text)}&_t=${Date.now()}`}catch(e){}};const flush=()=>{clearTimeout(timer);timer=null;while(queue.length){const batch=[];let len=ENDPOINT.length+50;while(queue.length){const item=queue[0];const est=item.length*3;if(len+est>MAX_URL&&batch.length)break;batch.push(queue.shift());len+=est}send(batch)}};const schedule=()=>{if(unloading)return flush();if(!timer)timer=setTimeout(flush,FLUSH_MS)};const orig={};['log','info','warn','error','debug'].forEach(m=>{orig[m]=console[m];console[m]=function(...args){orig[m].apply(console,args);queue.push(cut(toText(args),MAX_MSG));if(queue.length>MAX_QUEUE)queue.shift();schedule()}});window.addEventListener('error',e=>{queue.push(`[Error]${e.message}at ${e.filename}:${e.lineno}`);schedule()});window.addEventListener('unhandledrejection',e=>{const r=e.reason;const msg=r instanceof Error?(r.stack||r.message):stringify(r);queue.push(`[Unhandled]${msg}`);schedule()});window.addEventListener('beforeunload',()=>{unloading=true;flush()})})();__tt_define__(",
                @"useRegex": @NO
            }
        ]
    }];
