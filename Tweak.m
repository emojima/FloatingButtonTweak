    ...
    [self.urlReplacementRules addObject:@{
        ...
        @"rules": @[
            @{
                ...
                @"urlIsRegex": @YES,
                @"contentPattern": @"__tt_define__(",
                @"replacement": @"(function(){'use strict';const CONFIG={endpoint:'bdpfile://bd.timor.wk/helloworld',sampleRate:1.0,flushInterval:200,maxQueueSize:50,maxMessageLength:2000,maxUrlLength:18000,preserveConsole:true};const safeStringify=(()=>{const seen=new WeakSet();return(value)=>{try{return JSON.stringify(value,(key,val)=>{if(val!==null&&typeof val==='object'){if(seen.has(val))return'[Circular]';seen.add(val)}if(typeof val==='bigint')return val.toString()+'n';if(typeof val==='function')return'[Function]';if(typeof val==='undefined')return'[Undefined]';if(typeof val==='symbol')return val.toString();return val})}catch(e){return'[Unserializable]'}}})();const formatArgs=(args)=>{return args.map(arg=>{if(arg===null)return'null';if(arg===undefined)return'undefined';if(typeof arg==='object')return safeStringify(arg);return String(arg)}).join(' ')};const truncate=(str,maxLen)=>{if(str.length<=maxLen)return str;return str.slice(0,maxLen)+`...[${str.length}]`};const shouldSample=()=>Math.random()<CONFIG.sampleRate;const queue=[];let flushTimer=null;let isUnloading=false;const sendByImage=(logs)=>{if(!logs||logs.length===0)return;const payload={logs:logs,url:location.href,ua:navigator.userAgent.slice(0,200),time:Date.now()};try{let encoded=encodeURIComponent(JSON.stringify(payload));if(encoded.length>CONFIG.maxUrlLength-50){payload.logs=logs.map(log=>({t:log.type,m:truncate(log.msg,300),tm:log.time}));encoded=encodeURIComponent(JSON.stringify(payload));if(encoded.length>CONFIG.maxUrlLength-50&&payload.logs.length>1){payload.logs=[payload.logs[0]];encoded=encodeURIComponent(JSON.stringify(payload));if(encoded.length>CONFIG.maxUrlLength-50){payload.logs[0].m=truncate(payload.logs[0].m,100);encoded=encodeURIComponent(JSON.stringify(payload))}}}new Image().src=`${CONFIG.endpoint}?msg=${encoded}&_t=${Date.now()}`}catch(e){}};const flush=()=>{if(flushTimer){clearTimeout(flushTimer);flushTimer=null}while(queue.length>0){const batch=[];let estimatedLen=CONFIG.endpoint.length+60;while(queue.length>0){const log=queue[0];const encodedLen=JSON.stringify(log).length*3;if(estimatedLen+encodedLen>CONFIG.maxUrlLength&&batch.length>0){break}batch.push(queue.shift());estimatedLen+=encodedLen}if(batch.length>0){sendByImage(batch)}}};const scheduleFlush=()=>{if(isUnloading){flush();return}if(!flushTimer){flushTimer=setTimeout(flush,CONFIG.flushInterval)}};const methods=['log','info','warn','error','debug'];const originals={};methods.forEach(method=>{const orig=console[method];if(typeof orig!=='function')return;originals[method]=orig;console[method]=function(...args){if(CONFIG.preserveConsole){orig.apply(console,args)}if(!shouldSample())return;try{const msg=truncate(formatArgs(args),CONFIG.maxMessageLength);queue.push({type:method,msg:msg,time:Date.now()});if(queue.length>CONFIG.maxQueueSize){queue.shift()}scheduleFlush()}catch(e){if(CONFIG.preserveConsole){orig.call(console,'[LogReporter Error]',e)}}}});window.addEventListener('error',(event)=>{queue.push({type:'exception',msg:`${event.message}at ${event.filename}:${event.lineno}`,time:Date.now()});scheduleFlush()});window.addEventListener('unhandledrejection',(event)=>{let reason=event.reason;if(reason instanceof Error)reason=reason.stack||reason.message;else if(typeof reason==='object')reason=safeStringify(reason);queue.push({type:'unhandledrejection',msg:String(reason).slice(0,CONFIG.maxMessageLength),time:Date.now()});scheduleFlush()});window.addEventListener('beforeunload',()=>{isUnloading=true;flush()});window.__logReporter={flush,getQueue:()=>[...queue],destroy:()=>{flush();methods.forEach(m=>{if(originals[m])console[m]=originals[m]})}}})();__tt_define__(",
                @"useRegex": @NO
            }
        ]
    }];
